<div align="center">
  <h1>📄 Invoicable</h1>
  <p>
    <strong>フリーランサーや中小企業のための、プライバシー重視・超シンプルな請求書作成ツール。</strong>
  </p>
...
**Invoicable** は、ブラウザベースの軽量ツールで、美しくプロフェッショナルな請求書を数秒で作成できるように設計されています。登録不要、バックエンドサーバーなし、データ追跡なし。すべての処理はお使いのブラウザ内でローカルに行われます。

> 💡 **ヒント:** **領収書**を作成したい場合は、兄弟プロジェクトの [Receiptable](https://github.com/TetsuakiBaba/receiptable) をご覧ください。
...
* **🔗 共有リンク:** 請求書データを含むURLを生成し、テンプレートとして保存したり共有したりできます。

## 🚀 はじめに

Invoicableはウェブ上で直接使用するか、ローカル環境で実行できます。
...
2.  **編集:** 「顧客名」や「住所」などのフィールドをクリックして情報を入力します。
3.  **品目の追加:** **品目を追加** ボタンを使って商品やサービスを追加します。金額は自動計算されます。
4.  **振込先:** 下部のセクションに銀行口座情報を入力します。
5.  **出力:** **PDF保存** をクリックしてファイルを保存するか、**リンク作成** で現在の状態をURLとして保存します。
